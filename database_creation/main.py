...
num_views = 1
correct = np.zeros(shape=(20*num_views, len(test_folders)), dtype=np.bool)
...
labels_gr = ['Αεροπλάνο', 'Μπανιέρα', 'Κρεβάτι', 'Παγκάκι', 'Βιβλιοθήκη', 'Μπουκάλι', 'Μπολ', 'Αυτοκίνητο', 'Καρέκλα',
             'Κώνος', 'Κούπα', 'Κουρτίνα', 'Γραφείο', 'Πόρτα', 'Συρταριέρα', 'Βάζο λουλουδιών', 'Γυάλινο κουτί',
...
for i, im in enumerate(test_folders):
    ...
    # each object has 12 views, take only 4 random views
    k = 0
    # create object folder if it doesnt exist
    if not os.path.exists(os.path.join(save_folder, object_name)):
        os.makedirs(os.path.join(save_folder, object_name))
    for j in range(0, len(objects), 12):
        # random views to take
        views = random.sample(range(0, 12), num_views)
        views.sort()

        for v in views:
            # paths to currently processed image
            image_path = os.path.join(test_folders[i], objects[j+v])
            ...
            image.save(os.path.join(save_folder, object_name, objects[j+v]))
